...
extension ViewController: MapViewDelegate {
    func didChangedRegion(location: CLLocation) {
        if ((LocationServices.shared.centerMap == false)
             && (mapView.centerCoordinate.latitude != LocationServices.shared.position.value.coordinate.latitude)
             || (mapView.centerCoordinate.longitude != LocationServices.shared.position.value.coordinate.longitude)) {
